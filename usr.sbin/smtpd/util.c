...
#if !defined(NO_IO)
int io_xprintf(struct io *io, const char *fmt, ...) {
  va_list ap;
  int len;
...
int io_xprint(struct io *io, const char *str) {
  int len;
...
int rmtree(char *path, int keepdir) {
  char *path_argv[2];
  FTS *fts;
  FTSENT *e;
  int ret, depth;
...
int hostname_match(const char *hostname, const char *pattern) {
  while (*pattern != '\0' && *hostname != '\0') {
    if (*pattern == '*') {
      while (*pattern == '*')
        pattern++;
      while (*hostname != '\0' && tolower((unsigned char)*hostname) !=
                                      tolower((unsigned char)*pattern))
        hostname++;
      continue;
    }

    if (tolower((unsigned char)*pattern) != tolower((unsigned char)*hostname))
      return 0;
    pattern++;
    hostname++;
  }
...
int mailaddr_match(const struct mailaddr *maddr1,
                   const struct mailaddr *maddr2) {
  ...
  if (m2.user[0]) {
    ...
    if (strpbrk(m2.user, env->sc_subaddressing_delim) == NULL) {
      /* otherwise, strip tag from session address if any */
      p = strpbrk(m1.user, env->sc_subaddressing_delim);
      if (p)
        *p = '\0';
    }
...
int valid_localpart(const char *s) {
#define IS_ATEXT(c)                                                            \
  (isalnum((unsigned char)(c)) || strchr(MAILADDR_ALLOWED, (c)))
nextatom:
  if (!IS_ATEXT(*s) || *s == '\0')
    return 0;
  while (*(++s) != '\0') {
...
int valid_domainpart(const char *s) {
  struct in_addr ina;
...
#define LABELCHR(c)                                                            \
  ((c) == '-' || (c) == '_' || isalpha((unsigned char)(c)) ||                  \
   isdigit((unsigned char)(c)))
#define LABELMAX 63
#define DNAMEMAX 253

int valid_domainname(const char *str) {
  const char *label, *s;
...
int valid_smtp_response(const char *s) {
  if (strlen(s) < 5)
    return 0;

  if ((s[0] < '2' || s[0] > '5') || (s[1] < '0' || s[1] > '9') ||
      (s[2] < '0' || s[2] > '9') || (s[3] != ' '))
    return 0;
...
int secure_file(int fd, char *path, char *userdir, uid_t uid, int mayread) {
  char buf[PATH_MAX];
  ...
  if (fstat(fd, &st) == -1 || !S_ISREG(st.st_mode) || st.st_uid != uid ||
  ...
  for (;;) {
    ...
    if (stat(buf, &st) == -1 || (st.st_uid != 0 && st.st_uid != uid) ||
        (st.st_mode & 022) != 0)
      return 0;
...
int lowercase(char *buf, const char *s, size_t len) {
  if (len == 0)
    return 0;
...
int uppercase(char *buf, const char *s, size_t len) {
  if (len == 0)
    return 0;
...
void xlowercase(char *buf, const char *s, size_t len) {
  if (len == 0)
...
int session_socket_error(int fd) {
  int error;
  socklen_t len;
...
const char *parse_smtp_response(char *line, size_t len, char **msg, int *cont) {
  if (len >= LINE_MAX)
...
static int parse_mailname_file(char *hostname, size_t len) {
  FILE *fp;
  char *buf = NULL;
  size_t bufsz = 0;
  ssize_t buflen;
...
int getmailname(char *hostname, size_t len) {
...
int base64_encode(unsigned char const *src, size_t srclen, char *dest,
                  size_t destsize) {
  return __b64_ntop(src, srclen, dest, destsize);
}

int base64_decode(char const *src, unsigned char *dest, size_t destsize) {
  return __b64_pton(src, dest, destsize);
}

int base64_encode_rfc3548(unsigned char const *src, size_t srclen, char *dest,
                          size_t destsize) {
  size_t i;
  int ret;
...
void log_trace0(const char *emsg, ...) {
  va_list ap;
...
void portable_freeaddrinfo(struct addrinfo *ai) {
  struct addrinfo *p;
