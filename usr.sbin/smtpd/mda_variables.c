...
static ssize_t mda_expand_token(char *dest, size_t len, const char *token,
                                const struct deliver *dlv,
  ...
  if ((lbracket = strchr(rtoken, '[')) && (rbracket = strchr(rtoken, ']'))) {
    ...
    *lbracket = *rbracket = '\0';
    ...
    if ((sep = strchr(content, ':')) == NULL)
      endoff = begoff =
          strtonum(content, -EXPAND_BUFFER, EXPAND_BUFFER, &errstr);
    else {
      *sep = '\0';
      if (content != sep)
      ...
      if (*(++sep)) {
        if (errstr == NULL)
          endoff = strtonum(sep, -EXPAND_BUFFER, EXPAND_BUFFER, &errstr);
      }
    }
    if (errstr)
      return -1;
  ...
  if (!strcasecmp("sender", rtoken)) {
    if (snprintf(tmp, sizeof tmp, "%s@%s", dlv->sender.user,
                 dlv->sender.domain) >= (int)sizeof tmp)
      return -1;
    if (strcmp(tmp, "@") == 0)
      (void)strlcpy(tmp, "", sizeof tmp);
    string = tmp;
  } else if (!strcasecmp("rcpt", rtoken)) {
    if (snprintf(tmp, sizeof tmp, "%s@%s", dlv->rcpt.user, dlv->rcpt.domain) >=
        (int)sizeof tmp)
      return -1;
    if (strcmp(tmp, "@") == 0)
      (void)strlcpy(tmp, "", sizeof tmp);
    ...
    if (snprintf(tmp, sizeof tmp, "%s@%s", dlv->dest.user, dlv->dest.domain) >=
        (int)sizeof tmp)
      return -1;
    if (strcmp(tmp, "@") == 0)
      (void)strlcpy(tmp, "", sizeof tmp);
...
ssize_t mda_expand_format(char *buf, size_t len, const struct deliver *dlv,
  ...
  if (strncmp(pbuf, "~/", 2) == 0) {
    ...
    if (tmpret >= sizeof tmpbuf) {
      log_warnx("warn: user directory for %s too large", ui->directory);
      return 0;
    }
    ret += tmpret;
    ptmp += tmpret;
  ...
  for (; *pbuf && ret < sizeof tmpbuf; ret += tmpret) {
    ...
    *strchr(token, '}') = '\0';
    ...
    exptoklen =
        mda_expand_token(exptok, sizeof exptok, token, dlv, ui, mda_command);
    if (exptoklen == -1)
      return -1;
...
static int mod_lowercase(char *buf, size_t len) {
  char tmp[EXPAND_BUFFER];
...
static int mod_uppercase(char *buf, size_t len) {
  char tmp[EXPAND_BUFFER];
...
static int mod_strip(char *buf, size_t len) {
  char *tag, *at;
  unsigned int i;

  /* gilles+hackers -> gilles */
  if ((tag = strpbrk(buf, env->sc_subaddressing_delim)) != NULL) {
